...
**Domain-specific directory structure via `STRUCTURE.md` (convention, not enforced):** Depending on the domain the Agent API is deployed for (case file management, property management, a knowledge base, …), a fixed target layout can be useful — which directories exist, which `kind`s are expected where, what naming conventions apply. That structure is documented as an ordinary `STRUCTURE.md` file **at the tree root** — no new endpoint, no new ACL special-casing: agents read it via the already-generic `GET /file/STRUCTURE.md`.

Deliberately **not** in `_system/` — that's reserved for admin-only/security-relevant configuration (e.g. `acl.json`); structure documentation, by contrast, should be openly readable by everyone, regardless of how tightly `_system/` gets locked down by ACL later on. And deliberately **not enforced**: the Agent API doesn't validate writes against `STRUCTURE.md` — same reasoning as for `.skill` above: what counts as a "valid structure" is domain-specific, so that's the orchestration layer's job, not the generic infrastructure's (§1).

Example: [docs/example-structure.md](example-structure.md).
