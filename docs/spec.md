...
**Domänenspezifische Verzeichnisstruktur über `STRUCTURE.md` (Konvention, nicht durchgesetzt):** Je nach Domäne, in der die Agent-API eingesetzt wird (Fallaktenverwaltung, Immobilienverwaltung, Wissensdatenbank, …), kann eine feste Zielstruktur sinnvoll sein — welche Verzeichnisse es gibt, welche `kind`s dort erwartet werden, welche Namenskonventionen gelten. Diese Struktur wird als gewöhnliche `STRUCTURE.md`-Datei **an der Baumwurzel** dokumentiert — kein neuer Endpunkt, keine neue ACL-Sonderrolle: Agenten lesen sie über das ohnehin generische `GET /file/STRUCTURE.md`.

Bewusst **nicht** in `_system/` — das ist für admin-only/sicherheitsrelevante Konfiguration reserviert (z.B. `acl.json`); eine Struktur-Doku soll dagegen für jeden offen lesbar sein, unabhängig davon, wie eng `_system/` künftig per ACL gesperrt wird. Und bewusst **nicht durchgesetzt**: die Agent-API validiert keine Writes gegen `STRUCTURE.md` — dieselbe Begründung wie bei `.skill` weiter oben: was als „gültige Struktur" zählt, ist domänenspezifisch, also Sache der Orchestrierungsebene, nicht der generischen Infrastruktur (§1).

Beispiel: [docs/example-structure.md](example-structure.md).
